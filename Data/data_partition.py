...
def partition_data(partition, n_train, n_parties, train_label, beta = 0.5, skew_class = 2):
    ...
    traindata_cls_counts, js_matrix = record_net_data_stats(train_label, net_dataidx_map)
    data_distributions = traindata_cls_counts / traindata_cls_counts.sum(axis=1)[:,np.newaxis]
    return net_dataidx_map, traindata_cls_counts, data_distributions, js_matrix
...
def record_net_data_stats(y_train, net_dataidx_map):
    ...
    kl_matrix = calculate_js_divergence_matrix(temp)  # 1-JS
    print('1-JS:', kl_matrix)

    return net_cls_counts_npy, kl_matrix
