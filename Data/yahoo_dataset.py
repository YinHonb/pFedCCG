...
def dataset_read(dataset, base_path, batch_size, n_parties, partition, beta, skew_class):
    if dataset == "yahoo":
        traindata = torch.load(os.path.join('../Data/data/yahoo_answers_csv/sents/TRAIN_data.pth.tar'))
        testdata = torch.load(os.path.join('../Data/data/yahoo_answers_csv/sents/TEST_data.pth.tar'))
